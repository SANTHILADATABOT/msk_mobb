...
if ($wanted != "Medical") {
	$survey = $pdo_conn->prepare("SELECT survey_id,user_id,country_id,state_id,district_id,city_id,area_id,unique_no,completed_status,family_no,contact_no FROM fact_finding_form WHERE delete_status!='1' $query ORDER BY survey_id DESC");
	$survey->execute();
...
$query1 = ' and a.disease_no!=""';
if ($user_type == '2') {
	$query1 .= '  and a.country_id="' . $country_id . '"';
} else if ($user_type == '3') {
	$query1 .= '  and a.country_id="' . $country_id . '" and a.state_id="' . $state_id . '" ';
} else if ($user_type == '4') {
	$query1 .= '  and a.country_id="' . $country_id . '" and a.state_id="' . $state_id . '" and a.district_id="' . $district_id . '"';
} else if ($user_type == '5') {
	$query1 .= '  and a.country_id="' . $country_id . '" and a.state_id="' . $state_id . '" and a.district_id="' . $district_id . '" and a.city_id="' . $city_id . '"';
} else if ($user_type == '6') {
	$query1 .= '  and a.country_id="' . $country_id . '" and a.state_id="' . $state_id . '" and a.district_id="' . $district_id . '" and a.city_id="' . $city_id . '" and a.area_id="' . $area_id . '"';
}
//echo "SELECT DISTINCT a.country_id,a.district_id,a.state_id,a.city_id,a.area_id,a.survey_id,a.unique_no,a.family_no,b.disease_details,b.surgery_details,b.surgery_details_no,b.mon_exp_on_medicine,b.mon_exp_on_medicine_no,a.contact_no,a.user_id,a.completed_status FROM  fact_family_disease b left join fact_finding_form a ON a.unique_no=b.unique_no  WHERE  b.delete_status!='1' and a.delete_status!='1' $query1 ORDER BY a.survey_id DESC";
	...
	?>
		...
		<tr>
			...
			<td>
				<a onclick="edit_survey('<?php echo $get_value['survey_id']; ?>','<?php echo $get_value['user_id']; ?>','<?php echo $get_value['country_id'] ?>','<?php echo $get_value['state_id'] ?>','<?php echo $get_value['district_id'] ?>','<?php echo $get_value['city_id'] ?>','<?php echo $get_value['area_id'] ?>','<?php echo $get_value['unique_no'] ?>','<?php echo $get_value['completed_status'] ?>')" class="btn btn-primary btn-icon rounded-circle">
					<div><i class="fa fa-pencil"></i></div>
				</a>
