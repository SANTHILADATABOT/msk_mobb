...
// / echo "SELECT * FROM volunteer_otp WHERE mobile_number = '".$mobile."' AND otp = '".$otp."' ";
  $statement = $pdo_conn->prepare("SELECT * FROM usercreation WHERE mobile = '".$mobile."' AND password = '".$otp."' AND if_volunteer_user = 'volunteer'  AND active_status = '1' ");
  $statement->execute();
  $check = $statement->fetch();
  $count =$statement->rowCount();
  $fetched_otp = $check['password'];
  ...
  $user_type=$check['if_volunteer_user'];
